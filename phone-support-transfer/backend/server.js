...
// Serve frontend (single-server mode: one port, no proxy)
if (SERVE_FRONTEND) {
...
app.use((err, req, res, next) => {
  if (!res.headersSent) res.status(500).setHeader('Content-Type', 'application/json').end(JSON.stringify({ error: 'Server error', details: err.message }))
})

// Start server immediately; seed in background (never block listen on MongoDB)
const APP_PORT = SERVE_FRONTEND && process.env.USE_HTTPS === '1' ? (process.env.FRONTEND_PORT || 3000) : PORT
function startServer() {
  const HOST = process.env.HOST || '0.0.0.0'
  const useHttps = process.env.USE_HTTPS === '1'
  const certDir = path.join(__dirname, '.cert')
  const listenPort = useHttps && SERVE_FRONTEND ? APP_PORT : PORT
  if (useHttps) {
    try {
      const https = require('https')
      const key = fs.readFileSync(path.join(certDir, 'key.pem'))
      const cert = fs.readFileSync(path.join(certDir, 'cert.pem'))
      https.createServer({ key, cert }, app).listen(listenPort, HOST, () => {
        console.log(`ClearCycle running on https://localhost:${listenPort} (API + frontend)`)
        if (HOST === '0.0.0.0') console.log(`  Phone: https://<your-ip>:${listenPort} — accept cert once`)
      })
    } catch (e) {
      console.error('HTTPS failed (run ./scripts/gen-cert.sh first):', e.message)
      app.listen(listenPort, HOST, () => console.log(`Server listening on port ${listenPort}`))
    }
  } else {
    app.listen(listenPort, HOST, () => {
      console.log(`Server listening on port ${listenPort}`)
      if (HOST === '0.0.0.0') console.log('  (LAN access: <your-ip>:' + listenPort + ')')
    })
  }
}
